import 'package:educationadmin/Modals/SocketRequest.dart';
...
class ChatController extends GetxController
{
 ...
 {
  socket=io(SOCKET_URI,OptionBuilder().setTransports(['websocket']).build());
  connectSocket();
 }
 ...
 void connectSocket()
 {
  ...
  socket.onConnect((data) => logger.i("Connection established $data"));
   socket.onConnectError((data) => logger.e('Connect Error: $data'));
    socket.onDisconnect((data) => logger.i('Socket.IO server disconnected $data'));
  ...
  {
    SocketData socketRequest=SocketData(room: id.toString(),userName: userDetailsManager.username.value);
    socket.emit(JOIN_ROOM,socketRequest.toJson());
  }

  void sendMessage({required int id,required String message})
  {
    SocketData socketRequest=SocketData(room: id.toString(),userName: userDetailsManager.username.value,message: message);
