...
import 'package:talentsearchenglish/Modals/BannersResponse.dart';
import 'package:talentsearchenglish/Modals/BannersUploadResponse.dart';
import 'package:talentsearchenglish/Modals/ChannelCreationResponse.dart';
import 'package:talentsearchenglish/Modals/ChannelListModal.dart';
import 'package:talentsearchenglish/Modals/CreateNoticeResponse.dart';
import 'package:talentsearchenglish/Modals/EditResourceModal.dart';
import 'package:talentsearchenglish/Modals/FileResourcesModal.dart';
import 'package:talentsearchenglish/Modals/FileUploadResponseModal.dart';
import 'package:talentsearchenglish/Modals/ImageUploadResponse.dart';
import 'package:talentsearchenglish/Modals/LoginModal.dart';
import 'package:talentsearchenglish/Modals/NoticesResponse.dart';
import 'package:talentsearchenglish/Modals/PasswordResetResponse.dart';
import 'package:talentsearchenglish/Modals/PhoneVerificationModal.dart';
import 'package:talentsearchenglish/Modals/ProfileUpdateResponse.dart';
import 'package:talentsearchenglish/Modals/SignupResponseModal.dart';
import 'package:talentsearchenglish/Modals/SubsciberModal.dart';
import 'package:talentsearchenglish/Modals/UserModal.dart';
import 'package:talentsearchenglish/Modals/VideoRequestModal.dart';
import 'package:talentsearchenglish/Modals/VideoResourcesModal.dart';
import 'package:talentsearchenglish/Modals/VideoUploadResponse.dart';
import 'package:talentsearchenglish/utils/status.dart';
...
//     }
    ...
    try {
      Response<Map<String, dynamic>> response =
          await post('$baseURL$signup', map);
      logger.e(response.body.toString());
      if (response.body != null) {
        if (response.body!["statusCode"] == Status.positive ||
            response.body!["statusCode"] == Status.created) {
          return SignupResponseModal.fromJson(response.body!);
        } else {
          Map<String, dynamic> map = response.body!['error'];
          return SignupResponseModal(
              data: null, msg: map['msg'], statusCode: map['statusCode']);
        }
      } else {
        return SignupResponseModal(
            data: null,
  ...
  Future<OTPModal?> sendOTP({required PhoneModal phone}) async {
    try {
      Response<Map<String, dynamic>> response =
          await post('$baseURL$sendotp', phone.toJson());
  ...
  Future<UserModal> getUserDetails({String? token}) async {
    try {
      Response<Map<String, dynamic>> response = await get('$baseURL$user',
  ...
  Future<ChannelListModal> getAllChannelList({String? token}) async {
    try {
      Response<Map<String, dynamic>> response = await get('$baseURL$channels',
          headers: {"Authorization": "Bearer $token"});
      logger.e(response.body);
  ...
  Future<ChannelListModal> getCreatorList({String? token}) async {
    try {
      try {
        Response<Map<String, dynamic>> response = await get(
            '$baseURL$creatorChannels',
            headers: {"Authorization": "Bearer $token"});
        logger.e(response.body);
        if (response.body != null) {
          return ChannelListModal.fromJson(response.body!);
  ...
  Future<ChannelListModal> getUserChannelList(
      {String? token, required int userid}) async {
    try {
      Response<Map<String, dynamic>> response = await get(
          '$baseURL$userchannels',
          headers: {"Authorization": "Bearer $token"});
      if (response.body != null) {
        return ChannelListModal.fromJson(response.body!);
      } else {
  ...
  Future<FileResourcesData> getChannelFiles(
      {String? token, required int channelId}) async {
    Response<Map<String, dynamic>> response = await get(
        '$baseURL$channelfiles$channelId',
        headers: {"Authorization": "Bearer $token"});
    logger.e(response.body);
    return FileResourcesData.fromJson(response.body!);
  }

  Future<VideoResourcesData> getChannelVideo(
      {String? token, required int channelId}) async {
    try {
      Response<Map<String, dynamic>> response = await get(
          '$baseURL$channelvideos$channelId',
          headers: {"Authorization": "Bearer $token"});
      logger.e(response.body);
      if (response.body != null) {
  ...
  Future<CreateChannelResponseModal> createChannel(
      {required String? token,
      required XFile file,
      required bool isCompletelyPaid,
    ...
    try {
      ...
      dio.Response<Map<String, dynamic>> response = await dio.Dio().post(
  ...
  Future<CreateChannelResponseModal> editChannel(
      {required bool paidChannel,
      required int price,
      required String? token,
      XFile? file,
      required String name,
    ...
    try {
      //Response<Map<String,dynamic>> response=await post('$baseURL$createchannel', formData,contentType:"multipart/form-data",headers: {"Authorization":"Bearer $token"} );
      dio.Response<Map<String, dynamic>> response = await dio.Dio().put(
          '$baseURL$editchannel/$channelid',
          data: formData,
          options: dio.Options(
              contentType: "multipart/form-data",
              headers: {"Authorization": "Bearer $token"}));
  ...
  Future<bool> deleteChannel(
      {required String token, required int channelId}) async {
    Response<Map<String, dynamic>> response = await delete(
        "$baseURL$deletechannel$channelId",
        headers: {"Authorization": "Bearer $token"});
  ...
  Future<GeneralResponse2> subscribeByCreator(
      {required String token,
      required int channelId,
      required int consumerId}) async {
    try {
      Response<Map<String, dynamic>> response = await post(
          "$baseURL$addconsumerfromcreator/$channelId/$consumerId", {},
          headers: {"Authorization": "Bearer $token"});
  ...
  Future<GeneralResponse2> removeSubscriber(
      {required String token,
      required int channelId,
      required int consumerId}) async {
    try {
      Response<Map<String, dynamic>> response = await post(
          "$baseURL$removesubscriber$channelId/$consumerId", {},
          headers: {"Authorization": "Bearer $token"});
      logger.i(response.body);
      if (response.statusCode == 200 || response.statusCode == 201) {
        if (response.body != null) {
  ...
  Future<VideoUploadResponseModal> uploadVideo(
      {required String token,
      required VideoRequestModal videoRequestModal}) async {
    try {
      Response<Map<String, dynamic>> response = await post(
          '$baseURL$addresource', videoRequestModal.toJson(),
          headers: {"Authorization": "Bearer $token"});
      logger.e(response.body);
      if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      return VideoUploadResponseModal(
          data: VideoUploadData(msg: "Could not upload video", resource: null));
    }
    return VideoUploadResponseModal(
        data: VideoUploadData(msg: "Could not upload video", resource: null));
  }

  Future<VideoUploadResponseModal> uploadFileData(
      {required String token,
      required VideoRequestModal videoRequestModal}) async {
    try {
      Response<Map<String, dynamic>> response = await post(
  ...
  Future<SubscriberListModal> getChannelSubcribers(
      {required String token, required int channelId}) async {
    logger.i("Channel id: $channelId");

    try {
      Response<Map<String, dynamic>> response = await get(
          '$baseURL$channelsubscribers$channelId',
          headers: {"Authorization": "Bearer $token"});
      logger.e(response.body);
      if (response.statusCode == 200 || response.statusCode == 201) {
  ...
  Future<EditResourceModal> editResource(
      {required String token,
      required int resourceId,
      required String title}) async {
    try {
      Response response = await put(
          '$baseURL$editresource$resourceId', {"title": title},
          headers: {"Authorization": "Bearer $token"});
      ...
      return EditResourceModal(
          data: EditedData(msg: "Could not update ", updatedResource: null));
    }
  }

  Future<GeneralResponse> deleteresource(
      {required String token, required int resourceId}) async {
    try {
      Response response = await delete('$baseURL$deleteResource$resourceId',
          headers: {"Authorization": "Bearer $token"});
      ...
      return GeneralResponse(
          msg: 'Could not delete the resource', status: false);
    }
  }
  ...
  Future<PasswordResetResponse> resetPassword(
    ...
    try {
      Response<Map<String, dynamic>> response =
          await post("$baseURL$resetpassword", body);
      logger.e(response.body);
      if (response.body != null) {
  ...
  Future<BannersResponse> getBanners({required String jwt}) async {
    try {
      Response<Map<String, dynamic>> response = await get("$baseURL$getbanners",
  ...
  Future<BannersUploadResponse> uploadBannerImage(
      {required String token, required XFile? file}) async {
    var formData = dio.FormData.fromMap({
      'file': await dio.MultipartFile.fromFile(file!.path, filename: file.name),
    });
  ...
  Future<NoticesResponse> getNotices({required String token}) async {
    try {
      Response response = await get("$baseURL$getnotices",
          headers: {"Authorization": "Bearer $token"});
      logger.e(response.body);
      if (response.body != null) {
  ...
  Future<CreateNoticeResponse> createNotices(
    ...
    try {
      Response response = await post("$baseURL$createnotice", body,
          headers: {"Authorization": "Bearer $token"});
      if (response.body != null) {
        logger.e(response.body);
  ...
  Future<GeneralResponse2> updateProfile(
      {required SignupModal signupModal,
      required String token,
    ...
    try {
      Response response = await put("$baseURL$updateprofile", map,
          headers: {"Authorization": "Bearer $token"});
      if (response.body != null) {
        logger.e(response.body);
        return GeneralResponse2.fromJson(response.body);
  ...
  Future<GeneralResponse2> deleteNotice(
      {required int id, required String token}) async {
    try {
      Response response = await delete("$baseURL$deletenotice$id",
          headers: {"Authorization": "Bearer $token"});
      if (response.body != null) {
        return GeneralResponse2.fromJson(response.body);
      } else {
  ...
  Future<GeneralResponse2> deleteBanner(
      {required int id, required String token}) async {
    try {
      Response response = await delete("$baseURL$deletebanner$id",
          headers: {"Authorization": "Bearer $token"});
      if (response.body != null) {
        return GeneralResponse2.fromJson(response.body);
      } else {
  ...
  Future<GeneralResponse2> startStream(
      {required int id, required String token, required String title}) async {
    Map<String, dynamic> body = {"title": title};
    try {
      Response response = await put("$baseURL$startstream$id", body,
          headers: {"Authorization": "Bearer $token"});
      logger.i(response.body);
      if (response.body != null) {
        return GeneralResponse2.fromJson(response.body);
  ...
  Future<GeneralResponse2> endStream(
      {required int id, required String token, required String title}) async {
    Map<String, dynamic> body = {"title": title};
    try {
      Response response = await put("$baseURL$endstream$id", body,
          headers: {"Authorization": "Bearer $token"});
      logger.i(response.body);
      if (response.body != null) {
        return GeneralResponse2.fromJson(response.body);
