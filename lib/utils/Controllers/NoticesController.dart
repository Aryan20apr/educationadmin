...
class NoticesController extends GetxController {
  ...
  Future<void> createNotice(
    ...
    if (isAvailable) {
      ...
      notice.CreateNoticeResponse response = await networkService.createNotices(
  ...
  Future<void> deleteNotice({required int id, required int index}) async {
    String? token = await authenticationManager.getToken();
    General.GeneralResponse2 generalResponse2 =
        await networkService.deleteNotice(id: id, token: token!);
    if (generalResponse2.data == null) {
      Get.showSnackbar(const GetSnackBar(
        message: "Cannot delete notices",
        duration: Duration(seconds: 3),
      ));
    } else {
