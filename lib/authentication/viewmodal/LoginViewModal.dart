...
import 'package:talentsearchenglish/authentication/OTPVerification.dart';
import 'package:talentsearchenglish/authentication/ResetPassword.dart';
import 'package:talentsearchenglish/authentication/SignupScreen.dart';
...
class AuthenticationViewModal extends GetxController {
  ...
  Future<void> login(
      {String? email, required String password, required phone}) async {
    isSiginingIn.value = true;
    LoginModal loginModal = LoginModal(password: password, phone: phone);
    final SignupResponseModal response = await networkService.login(loginModal);
    ...
    if (response.data != null) {
      ...
      UserModal userModal =
          await _networkService.getUserDetails(token: response.data!.token);
  ...
  Future<void> registerUser(
    ...
    SignupModal signupModal =
        SignupModal(name: name, email: email, password: password, phone: phone);
    final SignupResponseModal response =
        await networkService.signUp(signupModal);
  ...
  Future<void> sendOtp(
    ...
    final OTPModal? response = await networkService.sendOTP(phone: phonemodal);
  ...
  Future<void> changePassword(
    ...
    if (verificationType == VerificationType.ForgotPassword) {
      PasswordResetResponse passwordResetResponse = await networkService
          .resetPassword(phone: phonenumber!, password: password);
      if (passwordResetResponse.data != null &&
          passwordResetResponse.data!.status!) {
      ...
      PasswordResetResponse passwordResetResponse = await networkService
          .resetPassword(phone: _userdetails.phone.value, password: password);
      if (passwordResetResponse.data != null &&
          passwordResetResponse.data!.status!) {
        Get.showSnackbar(const GetSnackBar(
          message: "Password changed successfully",
